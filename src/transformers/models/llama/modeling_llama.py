...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.tokenized_system_prompt = np.array([128000, 128006,   9125, 128007, 271])
        #hard coded thought prompt
        self.tokenized_thought_prompt = np.array([271])
        #hard coded response prompt
        self.tokenized_response_prompt = np.array([])
        
        #these will need adjusted in the intercept to allow for token insertion
        ...
        self.system_suffix_thoughts_first = np.array([198, 40, 1097, 5131, 304, 279, 3463, 11008, 271])
    ...
    def prepare_inputs_for_thoughts(
        ...
        #remove first character of processed inputs
        processed_inputs = processed_inputs[1:]
    
        # Concatenate all components
        components = [
            self.tokenized_system_prompt,
            self.system_suffix_thoughts,
            self.eot_tensor,
            self.bridge_tensor,
        ...
        def convert_to_numpy(comp):
            if isinstance(comp, np.ndarray):
                return comp.flatten()  # Flatten numpy arrays
            elif isinstance(comp, list):
                if len(comp) > 0 and isinstance(comp[0], dict):
                    # Handle list of dictionaries (sense_of_self and sense_of_user)
                    return np.concatenate([np.array(item).flatten() for item in comp])
                return np.array(comp).flatten()  # Flatten lists
            elif isinstance(comp, dict):
                # If it's a dictionary, we'll concatenate all its values
                ...
                return comp.cpu().numpy().flatten()  # Flatten torch tensors
            else:
        ...
        for i, comp in enumerate(components):
            try:
                ...
                logger.error(f"Error processing component {i}: {str(e)}")
                logger.error(f"Problematic component: {comp}")

        # Concatenate the converted components
        try:
            new_processed_inputs = np.concatenate(converted_components).astype(np.int32)
        except Exception as e:
            logger.error(f"Error during concatenation: {str(e)}")
            logger.error(f"Shapes of components: {[comp.shape for comp in converted_components]}")
            raise
        ...
        #logger.info(f"processed_inputs at the end of prepare_inputs_for_thoughts: \n\n{new_processed_inputs}\n\n")
    ...
    def prepare_inputs_for_response(
        ...
        for array in [self.tokenized_system_prompt_first, self.system_suffix_response_first, thoughts, self.bridge_tensor, 
            ...
            #logger.info(f"filling array with shape: {array}")
    ...
    def forward(
        ...
        #check number of tokens to determine if this is intercept or passthrough
        if input_ids.size(1) == 1:
            pass
            #handle context positions
            #position 0: nothing
            #position 1: save thoughts (experimental)
            #position 2: nothing
            #position 3: save ai response (experimental)
        else:
            logger.info("\n\n============")
            logger.info(f"Starting Forward Pass with context_position: {self.context_position}\n\n\n")
            
            if self.context_position == 0:
                thoughts = []
                processed_inputs =[]
    
                processed_inputs, attention_mask = self.prepare_inputs_for_thoughts(input_ids, attention_mask)
                #convert to text
        ...
        #logger.info(f"processed input_ids before intercept: {input_ids}")
        #generate response
    ...
    def forward_intercept(
        ...
        #override cache_position if we are in the intercept
